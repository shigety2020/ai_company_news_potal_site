...
毎朝 07:00 JST にバックエンドの自動収集が、その日の `data/YYYY-MM-DD.json` を `feat/white-editorial-skeleton` に載せる（人手 push なし）。収集前に残高確認。失敗時はファイルを置かず、無い日の空号（`featured: null`＋空目次）を壊さない。
